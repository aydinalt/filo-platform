...
export const updateNotificationRetentionSchema=z.object({readRetentionDays:z.number().int().min(30).max(730),automaticArchiveEnabled:z.boolean(),archiveIntervalHours:z.number().int().min(1).max(168),archiveBatchSize:z.number().int().min(1).max(5000)});
...
export const reconcileNotificationArchiveAttemptsSchema=z.object({tenantId:z.string().uuid(),actorUserId:z.string().uuid(),reconciliationKey:z.string().trim().regex(/^[A-Za-z0-9][A-Za-z0-9._:-]{7,119}$/),staleAfterMinutes:z.number().int().min(5).max(1440).default(15)});
