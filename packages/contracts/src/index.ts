...
export const updateNotificationRetentionSchema=z.object({readRetentionDays:z.number().int().min(30).max(730)});
...
export type NotificationArchiveRun={id:string;cutoffAt:string;retentionDays:number;archivedCount:number;initiatedBy:string;createdAt:string};
export type NotificationRetention={settings:{readRetentionDays:number;updatedAt:string|null};recentRuns:NotificationArchiveRun[]};
