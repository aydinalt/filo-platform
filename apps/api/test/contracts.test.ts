...
import { createNotificationProviderSchema, createNotificationSuppressionSchema, deliveryQuerySchema, notificationActionTargetSchema, notificationAnalyticsQuerySchema, notificationProviderHealthQuerySchema, notificationProviderIncidentQuerySchema, providerWebhookSchema, reconcileNotificationArchiveAttemptsSchema, retryNotificationArchiveSchema, runNotificationArchiveSchema, runNotificationProviderIncidentScanSchema, updateDeliveryStatusSchema, updateNotificationPreferencesSchema, updateNotificationProviderHealthSettingsSchema, updateNotificationProviderIncidentScanSettingsSchema, updateNotificationProviderIncidentSchema, updateNotificationRetentionSchema } from "@filo/contracts";
...
describe("API contracts", () => {
  ...
  it("bounds idempotent stale archive attempt reconciliation",()=>{const input={tenantId:"10000000-0000-4000-8000-000000000001",actorUserId:"20000000-0000-4000-8000-000000000001",reconciliationKey:"reconcile:2026-08-04T16:30",staleAfterMinutes:15};assert.equal(reconcileNotificationArchiveAttemptsSchema.safeParse(input).success,true);assert.equal(reconcileNotificationArchiveAttemptsSchema.safeParse({...input,reconciliationKey:"short"}).success,false);assert.equal(reconcileNotificationArchiveAttemptsSchema.safeParse({...input,staleAfterMinutes:4}).success,false);assert.equal(reconcileNotificationArchiveAttemptsSchema.safeParse({...input,staleAfterMinutes:1441}).success,false);});
