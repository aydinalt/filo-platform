...
describe("API contracts", () => {
  it("bounds notification retention and exposes the archive inbox filter",()=>{assert.equal(updateNotificationRetentionSchema.safeParse({readRetentionDays:90}).success,true);assert.equal(updateNotificationRetentionSchema.safeParse({readRetentionDays:7}).success,false);assert.equal(notificationQuerySchema.safeParse({status:"archived"}).success,true);});
