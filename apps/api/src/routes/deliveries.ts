...
export async function deliveryRoutes(app:FastifyInstance){
 ...
 app.post("/enqueue",{preHandler:[requireSession,allow("owner","admin","operator")]},async request=>withTenantTransaction(request.sessionUser.tenantId,request.sessionUser.id,async c=>{
  const candidates=(await c.query(`SELECT n.id,n.tenant_id AS "tenantId",n.recipient_user_id AS "recipientUserId",n.source_type AS "sourceType",n.title,n.message,u.full_name AS "recipientName",p.timezone,p.quiet_hours_enabled AS "quietEnabled",p.quiet_start AS "quietStart",p.quiet_end AS "quietEnd",x.channel FROM in_app_notifications n JOIN users u ON u.id=n.recipient_user_id JOIN notification_preferences p ON p.tenant_id=n.tenant_id AND p.user_id=n.recipient_user_id CROSS JOIN LATERAL (VALUES('email',p.email_enabled),('push',p.push_enabled)) x(channel,enabled) WHERE x.enabled AND n.archived_at IS NULL AND n.source_type NOT IN ('provider_incident','archive_reconciliation') AND n.created_at>=now()-interval '30 days' AND NOT EXISTS(SELECT 1 FROM notification_delivery_outbox o WHERE o.notification_id=n.id AND o.recipient_user_id=n.recipient_user_id AND o.channel=x.channel)`)).rows;
  let created=0;
  for(const item of candidates){
   const locale="tr-TR",key=`notification.${item.sourceType}`;
   const template=(await c.query(`SELECT id,subject_template AS "subjectTemplate",body_template AS "bodyTemplate" FROM notification_templates WHERE key=$1 AND channel=$2 AND status='active' AND locale IN ($3,'tr-TR') ORDER BY (locale=$3) DESC LIMIT 1`,[key,item.channel,locale])).rows[0];
   ...
   const result=await c.query(`INSERT INTO notification_delivery_outbox(tenant_id,notification_id,recipient_user_id,channel,available_at,template_id,locale,rendered_subject,rendered_body) VALUES($1,$2,$3,$4,CASE WHEN $5 AND LOCALTIME>=$6 AND LOCALTIME<$7 THEN CURRENT_DATE+$7+interval '1 day' ELSE now() END,$8,$9,$10,$11) ON CONFLICT DO NOTHING`,[item.tenantId,item.id,item.recipientUserId,item.channel,item.quietEnabled,item.quietStart,item.quietEnd,template?.id??null,locale,subject,body]);
