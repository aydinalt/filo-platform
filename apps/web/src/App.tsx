...
import type { NotificationAnalytics, NotificationDelivery, NotificationPreferences, NotificationProviderHealth, NotificationProviderIncident, NotificationProviderIncidentScanStatus, NotificationTemplate } from "@filo/contracts";
import { api } from "./api";
...
function Dashboard({ user, onLogout }: { user: SessionUser; onLogout: () => void }) {
  ...
  async function refresh() {
    ...
    try {
      ...
      if(user.role!=="viewer") { setNotificationAnalytics((await api.notificationAnalytics()).analytics); setNotificationProviderHealth(await api.notificationProviderHealth()); const providerIncidentResult=await api.notificationProviderIncidents();setNotificationProviderIncidents(providerIncidentResult.incidents);setNotificationProviderIncidentScanStatus(providerIncidentResult.scanStatus); }
