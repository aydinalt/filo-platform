...
import type { NotificationAnalytics, NotificationDelivery, NotificationPreferences, NotificationProviderHealth, NotificationProviderIncident, NotificationProviderIncidentScanStatus, NotificationRetention, NotificationTemplate } from "@filo/contracts";
import { api } from "./api";
...
function ProviderIncidentsPanel({ incidents, scanStatus, focusedId, onSync, onUpdate }: { incidents: NotificationProviderIncident[]; scanStatus: NotificationProviderIncidentScanStatus | null; focusedId: string | null; onSync: () => Promise<void>; onUpdate: (id: string, status: "acknowledged" | "resolved", notes: string | null) => Promise<void> }) {
  return <section className="table-card spaced">
    <div className="section-head"><div><p className="eyebrow">SAĞLAYICI OLAY YÖNETİMİ</p><h2>Operasyon olayları</h2><small>{scanStatus?.lastScanAt?`Son tarama: ${new Date(scanStatus.lastScanAt).toLocaleString("tr-TR")} · ${scanStatus.intervalMinutes} dk aralık`:"Henüz sağlık taraması yapılmadı"}</small></div><button onClick={onSync}>Şimdi tara</button></div>
    <div className="table-wrap"><table><thead><tr><th>Sağlayıcı</th><th>Sorun</th><th>Önem</th><th>Durum</th><th>Son kontrol</th><th>İşlem</th></tr></thead><tbody>{incidents.map(incident=><tr id={`provider-incident-${incident.id}`} className={focusedId===incident.id?"focused-row":undefined} key={incident.id}><td><b>{incident.providerName}</b><br/><small>{incident.channel} · {incident.provider}</small></td><td>{incident.issueTypes.join(", ")}<br/><small>{incident.occurrenceCount} kez algılandı</small></td><td>{incident.severity}</td><td>{incident.recoveryCandidateAt?"Çözülebilir":incident.status}<br/>{incident.recoveryCandidateAt&&<small>{incident.healthyScanCount} ardışık sağlıklı tarama</small>}</td><td>{incident.lastCheckedAt?new Date(incident.lastCheckedAt).toLocaleString("tr-TR"):new Date(incident.lastDetectedAt).toLocaleString("tr-TR")}</td><td>{incident.status==="open"&&<button className="secondary" onClick={()=>onUpdate(incident.id,"acknowledged",null)}>Gördüm</button>}{incident.status!=="resolved"&&<button onClick={async()=>{const notes=window.prompt(incident.recoveryCandidateAt?"Sağlık normale döndü. Çözüm notu":"Çözüm notu");if(notes)await onUpdate(incident.id,"resolved",notes);}}>Çöz</button>}</td></tr>)}</tbody></table></div>
...
function Dashboard({ user, onLogout }: { user: SessionUser; onLogout: () => void }) {
  ...
  const [notificationProviderIncidentScanStatus,setNotificationProviderIncidentScanStatus]=useState<NotificationProviderIncidentScanStatus|null>(null);
  const [notificationRetention,setNotificationRetention]=useState<NotificationRetention|null>(null);
  const [focusedProviderIncidentId,setFocusedProviderIncidentId]=useState<string|null>(null);
  ...
  const [view, setView] = useState<"overview" | "vehicles" | "drivers" | "devices" | "operations" | "geofences" | "alerts" | "maintenance" | "expenses" | "documents" | "safety" | "inspections" | "tires" | "incidents" | "reports" | "actions" | "notifications" | "mobile" | "members" | "audit">("overview");
  const [error, setError] = useState("");
  ...
  async function refresh() {
    setError("");
    try {
      const [vehicleResult, auditResult, driverResult, deviceResult,assignmentResult,shiftResult,trackingResult,locationResult,geofenceResult,geofenceEventResult,alertRuleResult,alertResult,maintenanceResult,expenseResult,documentResult,safetyResult,inspectionResult,tireResult,incidentResult,actionResult,notificationResult,notificationRuleResult,preferenceResult,deliveryResult,templateResult] = await Promise.all([api.vehicles(), api.auditEvents(), api.drivers(), api.devices(),api.assignments(),api.shifts(),api.tracking(),api.latestLocations(),api.geofences(),api.geofenceEvents(),api.alertRules(),api.alerts(),api.maintenancePlans(),api.expenses(),api.documents(),api.safetyEvents(),api.inspections(),api.tires(),api.incidents(),api.actions(),api.notifications(),api.notificationRules(),api.notificationPreferences(),user.role==="viewer"?Promise.resolve({deliveries:[]}):api.notificationDeliveries(),user.role==="viewer"?Promise.resolve({templates:[]}):api.notificationTemplates()]);
      ...
      if(user.role!=="viewer") { setNotificationAnalytics((await api.notificationAnalytics()).analytics); setNotificationProviderHealth(await api.notificationProviderHealth()); const providerIncidentResult=await api.notificationProviderIncidents();setNotificationProviderIncidents(providerIncidentResult.incidents);setNotificationProviderIncidentScanStatus(providerIncidentResult.scanStatus);setNotificationRetention(await api.notificationRetention()); }
  ...
  async function addTireSet(){
    const brand=window.prompt("Lastik markası");if(!brand)return;const model=window.prompt("Lastik modeli");if(!model)return;
    const size=window.prompt("Ebat (ör. 215/65 R16)");if(!size)return;const serialNumber=window.prompt("Seri/lot numarası (opsiyonel)")||null;
    const targetText=window.prompt("Hedef kullanım kilometresi (opsiyonel)","50000")||"";const targetChangeDate=window.prompt("Hedef değişim tarihi (YYYY-AA-GG, opsiyonel)")||null;
    try{await api.createTireSet({brand,model,size,serialNumber,purchasedOn:new Date().toISOString().slice(0,10),initialOdometerKm:null,targetLifeKm:targetText?Number(targetText):null,targetChangeDate,notes:null});await refresh();}
    catch(e){setError(e instanceof Error&&e.message==="TIRE_SERIAL_EXISTS"?"Bu seri numarası daha önce kaydedilmiş.":"Lastik seti kaydedilemedi; ebat ve hedef bilgilerini kontrol edin.");}
  }
  return <div className="app-shell">
    <aside><div className="brand"><span>F</span> Filo</div><nav>
      ...
      <button className={view === "notifications" ? "active" : ""} onClick={() => setView("notifications")}>● Bildirimler {notifications.filter(n=>!n.readAt).length?`(${notifications.filter(n=>!n.readAt).length})`:""}</button>
    ...
    <main className="dashboard">
      ...
      {view === "tires" && <>
        <section className="metrics">
          <article><span>Toplam set</span><strong>{tireSummary.total}</strong><small>lastik envanteri</small></article>
          <article><span>Takılı</span><strong>{tireSummary.mounted}</strong><small>aktif kullanımda</small></article>
          <article><span>Değişim yaklaşıyor</span><strong>{tireSummary.dueSoon}</strong><small>30 gün / 2.000 km</small></article>
          <article><span>Gecikmiş</span><strong>{tireSummary.overdue}</strong><small>müdahale gerekli</small></article>
        </section>
        <section className="table-card">
      ...
      {view === "notifications" && <>{user.role!=="viewer"&&<ProviderIncidentsPanel incidents={notificationProviderIncidents} scanStatus={notificationProviderIncidentScanStatus} focusedId={focusedProviderIncidentId} onSync={async()=>{await api.syncNotificationProviderIncidents();await refresh();}} onUpdate={async(id,status,notes)=>{await api.updateNotificationProviderIncident(id,status,notes);await refresh();}}/>}<section className="table-card spaced">
        <div className="section-head"><div><p className="eyebrow">UYGULAMA İÇİ BİLDİRİM KUTUSU</p><h2>Bildirimler</h2></div><div>{notifications.some(item=>!item.readAt)&&<button className="secondary" onClick={async()=>{await api.markAllNotificationsRead();await refresh();}}>Tümünü okundu yap</button>}{user.role!=="viewer"&&<button onClick={async()=>{await api.generateNotifications();await refresh();}}>Bildirimleri üret</button>}</div></div>
        <div className="table-wrap"><table><thead><tr><th>Önem</th><th>Bildirim</th><th>Araç</th><th>Zaman</th><th>Durum</th></tr></thead><tbody>{notifications.map(item=><tr key={item.id}><td><b>{item.severity}</b></td><td>{item.title}<br/><small>{item.message}</small>{item.actionTarget?.type==="provider_incident"&&<><br/><button className="secondary action-link" onClick={async()=>{if(!item.readAt)await api.markNotificationRead(item.id);setFocusedProviderIncidentId(item.actionTarget!.id);await refresh();setTimeout(()=>document.getElementById(`provider-incident-${item.actionTarget!.id}`)?.scrollIntoView({behavior:"smooth",block:"center"}),0);}}>Olaya git</button></>}</td><td>{item.vehiclePlate??"—"}</td><td>{new Date(item.createdAt).toLocaleString("tr-TR")}</td><td>{item.readAt?"Okundu":<button onClick={async()=>{await api.markNotificationRead(item.id);await refresh();}}>Okundu işaretle</button>}</td></tr>)}</tbody></table></div>
      ...
      </section>{user.role!=="viewer"&&notificationRetention&&<section className="table-card spaced"><div className="section-head"><div><p className="eyebrow">SAKLAMA VE ARŞİV</p><h2>Okunmuş bildirim yaşam döngüsü</h2><small>{notificationRetention.eligibleCount} kayıt uygun · {notificationRetention.settings.automaticArchiveEnabled?`Otomatik, ${notificationRetention.settings.archiveIntervalHours} saatte bir`:`Otomasyon kapalı`} · Kayıtlar silinmez</small></div><div>{["owner","admin"].includes(user.role)&&<><button className="secondary" onClick={async()=>{const raw=window.prompt("Okunmuş bildirim saklama süresi (30–730 gün)",String(notificationRetention.settings.readRetentionDays));if(!raw)return;const days=Number(raw);if(!Number.isInteger(days)||days<30||days>730){setError("Saklama süresi 30–730 gün arasında tam sayı olmalıdır.");return;}await api.updateNotificationRetention({...notificationRetention.settings,readRetentionDays:days});await refresh();}}>Süreyi değiştir</button><button className="secondary" onClick={async()=>{await api.updateNotificationRetention({...notificationRetention.settings,automaticArchiveEnabled:!notificationRetention.settings.automaticArchiveEnabled});await refresh();}}>Otomasyon: {notificationRetention.settings.automaticArchiveEnabled?"Açık":"Kapalı"}</button></>}<button onClick={async()=>{if(!window.confirm("En fazla batch limiti kadar uygun bildirim arşivlensin mi? Kayıtlar silinmeyecek."))return;await api.archiveEligibleNotifications();await refresh();}}>Uygun kayıtları arşivle</button></div></div><p>Saklama: {notificationRetention.settings.readRetentionDays} gün · Batch: {notificationRetention.settings.archiveBatchSize} · Son çalışma: {notificationRetention.settings.lastArchiveAt?new Date(notificationRetention.settings.lastArchiveAt).toLocaleString("tr-TR"):"—"} · Sonraki: {notificationRetention.settings.nextDueAt?new Date(notificationRetention.settings.nextDueAt).toLocaleString("tr-TR"):"—"}</p><div className="table-wrap"><table><thead><tr><th>Çalıştırma</th><th>Kaynak</th><th>Kesim zamanı</th><th>Limit</th><th>Arşivlenen</th></tr></thead><tbody>{notificationRetention.recentRuns.map(run=><tr key={run.id}><td>{new Date(run.createdAt).toLocaleString("tr-TR")}</td><td>{run.source==="scheduler"?"Zamanlayıcı":"Manuel"}</td><td>{new Date(run.cutoffAt).toLocaleString("tr-TR")}</td><td>{run.batchSize}</td><td>{run.archivedCount}</td></tr>)}</tbody></table></div>{!notificationRetention.recentRuns.length&&<div className="empty"><b>Henüz arşiv çalıştırması yok</b><p>Süreyi dolduran okunmuş bildirimler ilk kontrollü çalıştırmada arşivlenir.</p></div>}<div className="section-head"><div><p className="eyebrow">ÇALIŞTIRMA DURUMU</p><h3>Son arşivleme denemeleri</h3></div><small>Ham hata ayrıntısı yerine güvenli sonuç kodları gösterilir.</small></div><div className="table-wrap"><table><thead><tr><th>Başlangıç</th><th>Kaynak</th><th>Durum</th><th>Sonuç kodu</th><th>Arşivlenen</th><th>İşlem</th></tr></thead><tbody>{notificationRetention.recentAttempts.map(attempt=><tr key={attempt.id}><td>{new Date(attempt.startedAt).toLocaleString("tr-TR")}</td><td>{attempt.source==="scheduler"?"Zamanlayıcı":attempt.source==="retry"?`Yeniden deneme ${attempt.retryNumber}/3`:"Manuel"}</td><td>{attempt.status==="succeeded"?"Başarılı":attempt.status==="failed"?"Başarısız":attempt.status==="skipped"?"Atlandı":"Çalışıyor"}</td><td>{attempt.outcomeCode??"—"}</td><td>{attempt.archivedCount??"—"}</td><td>{attempt.status==="failed"&&attempt.retryNumber<3&&["owner","admin"].includes(user.role)?<button className="secondary" onClick={async()=>{if(!window.confirm("Bu başarısız arşivleme denemesi kontrollü olarak yeniden çalıştırılsın mı?"))return;await api.retryNotificationArchive(attempt.id);await refresh();}}>Yeniden dene</button>:"—"}</td></tr>)}</tbody></table></div>{!notificationRetention.recentAttempts.length&&<div className="empty"><b>Henüz çalıştırma denemesi yok</b></div>}</section>}{user.role!=="viewer"&&notificationAnalytics&&<section className="table-card spaced"><div className="section-head"><div><p className="eyebrow">TESLİMAT ANALİTİĞİ</p><h2>Son {notificationAnalytics.rangeDays} gün</h2></div></div><p>Toplam: {notificationAnalytics.summary.total} · Teslim: {notificationAnalytics.summary.delivered} · Hatalı: {notificationAnalytics.summary.failed} · Kuyruk: {notificationAnalytics.summary.queued}</p></section>}{user.role!=="viewer"&&notificationProviderHealth&&<section className="table-card spaced"><div className="section-head"><div><p className="eyebrow">SAĞLAYICI SAĞLIĞI</p><h2>Teslimat kanalları</h2></div><small>{notificationProviderHealth.settings.lookbackHours} saatlik görünüm</small></div><div className="table-wrap"><table><thead><tr><th>Sağlayıcı</th><th>Kanal</th><th>Durum</th><th>Başarısızlık</th><th>Kuyruk yaşı</th><th>Son teslim</th></tr></thead><tbody>{notificationProviderHealth.providers.map(item=><tr key={item.id}><td><b>{item.name}</b><br/><small>{item.provider}</small></td><td>{item.channel}</td><td>{item.health==="healthy"?"Sağlıklı":"Uyarı"}</td><td>%{item.failureRatePercent}</td><td>{item.oldestReadyAgeSeconds} sn</td><td>{item.lastDeliveredAt?new Date(item.lastDeliveredAt).toLocaleString("tr-TR"):"—"}</td></tr>)}</tbody></table></div>{!notificationProviderHealth.providers.length&&<div className="empty"><b>Sağlayıcı profili yok</b></div>}</section>}<section className="table-card spaced"><div className="section-head"><div><p className="eyebrow">TESLİMAT TERCİHLERİ</p><h2>Kanallar ve sessiz saatler</h2></div><button onClick={async()=>{await api.updateNotificationPreferences({...notificationPreferences,emailEnabled:!notificationPreferences.emailEnabled,updatedAt:undefined} as never);await refresh();}}>E-posta: {notificationPreferences.emailEnabled?"Açık":"Kapalı"}</button></div><p>Push: {notificationPreferences.pushEnabled?"Açık":"Kapalı"} · Saat dilimi: {notificationPreferences.timezone} · Sessiz saat: {notificationPreferences.quietHoursEnabled?`${notificationPreferences.quietStart}–${notificationPreferences.quietEnd}`:"Kapalı"}</p></section>{user.role!=="viewer"&&<section className="table-card spaced"><div className="section-head"><div><p className="eyebrow">SAĞLAYICI BAĞIMSIZ OUTBOX</p><h2>Bildirim teslimatları</h2></div><button onClick={async()=>{await api.enqueueNotificationDeliveries();await refresh();}}>Teslimat kuyruğu üret</button></div><div className="table-wrap"><table><thead><tr><th>Bildirim</th><th>Alıcı</th><th>Kanal</th><th>Durum</th><th>Deneme</th></tr></thead><tbody>{notificationDeliveries.map(item=><tr key={item.id}><td>{item.title}</td><td>{item.recipientName}</td><td>{item.channel}</td><td>{item.status}</td><td>{item.attemptCount}</td></tr>)}</tbody></table></div></section>}{["owner","admin"].includes(user.role)&&<section className="table-card spaced"><div className="section-head"><div><p className="eyebrow">ÇOK DİLLİ İÇERİK</p><h2>Bildirim şablonları</h2></div><button onClick={async()=>{const key=window.prompt("Şablon anahtarı (ör. notification.action)");if(!key)return;await api.createNotificationTemplate({key,channel:"email",locale:"tr-TR",subjectTemplate:"{{title}}",bodyTemplate:"Merhaba {{recipientName}}, {{message}}",requiredVariables:["title","recipientName","message"]});await refresh();}}>＋ Şablon ekle</button></div><div className="table-wrap"><table><thead><tr><th>Anahtar</th><th>Kanal</th><th>Dil</th><th>Durum</th></tr></thead><tbody>{notificationTemplates.map(item=><tr key={item.id}><td><b>{item.key}</b></td><td>{item.channel}</td><td>{item.locale}</td><td><button className="secondary" onClick={async()=>{await api.updateNotificationTemplate(item.id,item.status==="active"?"inactive":"active");await refresh();}}>{item.status}</button></td></tr>)}</tbody></table></div></section>}{["owner","admin"].includes(user.role)&&<section className="table-card spaced"><div className="section-head"><div><p className="eyebrow">TENANT BİLDİRİM KURALLARI</p><h2>Kurallar</h2></div><button onClick={async()=>{const name=window.prompt("Kural adı");if(!name)return;await api.createNotificationRule({name,sourceType:"action",leadDays:7,severity:"warning",targetRole:null});await refresh();}}>＋ Kural ekle</button></div><div className="table-wrap"><table><thead><tr><th>Kural</th><th>Kaynak</th><th>Ön süre</th><th>Önem</th><th>Hedef</th><th>Durum</th></tr></thead><tbody>{notificationRules.map(rule=><tr key={rule.id}><td><b>{rule.name}</b></td><td>{rule.sourceType}</td><td>{rule.leadDays} gün</td><td>{rule.severity}</td><td>{rule.targetRole??"Tüm roller"}</td><td><button className="secondary" onClick={async()=>{await api.updateNotificationRule(rule.id,rule.status==="active"?"inactive":"active");await refresh();}}>{rule.status}</button></td></tr>)}</tbody></table></div></section>}</>}
