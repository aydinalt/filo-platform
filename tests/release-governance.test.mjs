...
test("release workflow contains quality, SCA, CodeQL, Supabase and reproducibility gates",async()=>{
  ...
  for(const marker of ["quality-and-artifact", "dependency-review", "software-composition", "codeql", "reproducible-build", "supabase-database", "supabase test db", "npm run ci:verify", "npm audit --omit=dev --audit-level=high"])assert.match(workflow,new RegExp(marker,"u"));
});
