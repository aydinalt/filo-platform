...
test("production preflight accepts a complete contract without revealing secrets",()=>{
  ...
  Object.assign(env,{FILO_DEPLOYMENT_TIER:"production",NEXT_PUBLIC_FILO_DEPLOYMENT_TIER:"production",FILO_DEMO_AUTH_ENABLED:"false",OPERATIONS_ALERT_EMAILS:"oncall@fleet.invalid",BROWSER_TELEMETRY_ENABLED:"false"});
...
test("production preflight accepts the Vercel Supabase contract without requiring D1 or R2",()=>{
  ...
  Object.assign(env,{FILO_DEPLOYMENT_TIER:"production",NEXT_PUBLIC_FILO_DEPLOYMENT_TIER:"production",FILO_DEMO_AUTH_ENABLED:"false",OPERATIONS_ALERT_EMAILS:"oncall@fleet.invalid",BROWSER_TELEMETRY_ENABLED:"false",PRIVILEGED_MFA_REQUIRED:"true"});
  const run=spawnSync(process.execPath,[resolve(root,"scripts/production-preflight.mjs"),"--json","--strict","--environment=production"],{cwd:root,env,encoding:"utf8"});
...
test("Supabase baseline generator stays immutable and converts SQLite-only audit triggers",async()=>{
  const generator=await readFile(resolve(root,"scripts/generate-supabase-migration.mjs"),"utf8");
  assert.match(generator,/INITIAL_BASELINE_MAX_INDEX = 12/);
  assert.match(generator,/block_audit_event_mutation/);
  assert.match(generator,/tenant_id.*NOT NULL/);
});
