...
test("Supabase runtime rejects Sites identity headers before reading them",async()=>{const [boundary,auth]=await Promise.all([importTypeScript("lib/auth-boundary.ts"),read("app/chatgpt-auth.ts")]);assert.equal(boundary.shouldAcceptSitesIdentityHeaders("supabase"),false);assert.equal(boundary.shouldAcceptSitesIdentityHeaders("SUPABASE"),false);assert.equal(boundary.shouldAcceptSitesIdentityHeaders(undefined),true);assert.match(auth,/if \(!shouldAcceptSitesIdentityHeaders\(runtime\)\) return getSupabaseUser\(\);[\s\S]*await headers\(\)/)});
